...
| **P2** | OpenAPI export for “platform SMS API” | **Implemented** — `yarn nx run notification-service:generate-openapi` writes `docs/openapi/notification-service.openapi.json` + `apps/notification-service/openapi.json`. Narrative: [`platform-sms-openapi.md`](platform-sms-openapi.md). |
